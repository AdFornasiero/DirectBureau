<div class="w-full sm:w-5/6 md:w-3/4 lg:w-2/3 mx-auto">
	...
	<?= form_open_multipart('', array('class'=>'w-full flex flex-wrap justify-center border shadow bg-gray-100 rounded py-4')) ?>
		<?php if(isset($added) && $added){ ?>
			<span class="w-3/4 md:w-1/2 mx-auto text-center text-sm font-semibold bg-green-200 border border-green-800 text-green-800 rounded-lg py-1">Le produit a bien été ajouté</span>
		<?php } ?>
		<div class="w-3/4 mb-4">
			<label for="reference" class="ml-2">Référence</label>
			<input name="reference" id="reference" class="w-full shadow border rounded px-1" type="text" value="<?= !isset($added) ? set_value('reference') : '' ?>"/>
		...
		<div class="w-3/4 mb-4">
			...
			<input name="label" id="label" class="w-full shadow border rounded px-1" type="text" value="<?= !isset($added) ? set_value('label') : '' ?>"/>
		...
		<div class="flex flex-wrap items-end w-3/4 mb-4">
			<div class="w-full sm:w-1/2">
				...
				<input name="price" id="price" class="w-full shadow border rounded px-1" type="text" value="<?= !isset($added) ? set_value('price') : '' ?>"/>
		...
		<div class="w-3/4 text-right">
			<?= form_submit('', "Ajouter", array('class'=>'shadow-lg rounded px-4 py-2 text-white bg-blue-700')) ?>
		</div>
	<?= form_close() ?>
</div>
